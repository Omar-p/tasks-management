import { LoginRequest, SignupRequest, User, AuthTokens } from '@/contexts/auth.types';
  ...
  if (!response.ok) {
    try {
      ...
      if (errorData.errors && Object.keys(errorData.errors).length > 0) {
        ...
        // General error
        throw new AuthApiError(errorData.detail || errorData.title || 'Request failed', response.status);
      }
    } catch (parseError) {
      // Fallback if JSON parsing fails - check if parseError is from JSON.parse
      if (parseError instanceof SyntaxError) {
        // JSON parsing failed, use response status text
        throw new AuthApiError(response.statusText || 'Request failed', response.status);
      }
  ...
  async signin(credentials: LoginRequest): Promise<AuthTokens> {
    try {
      ...
      if (error instanceof TypeError) {
        throw new AuthApiError('Unable to connect to the server. Please check your internet connection and try again.');
      }
      throw error;
    }
  },
